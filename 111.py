...
import json
import os
################################## 이게 진짜 찐 get_coordinates_with_mouse.py임 ###########################################################
################################## 이게 진짜 찐 get_coordinates_with_mouse.py임 ###########################################################
################################## 이게 진짜 찐 get_coordinates_with_mouse.py임 ###########################################################
################################## 이게 진짜 찐 get_coordinates_with_mouse.py임 ###########################################################
################################## 이게 진짜 찐 get_coordinates_with_mouse.py임 ###########################################################
...
########### 마우스로 신분증 이미지에 좌표를 찍어 테두리를 생성하고 그 형태로 crop 하는 코드 #######

pts = [] #마우스로 클릭한 포인트를 저장
resultforJSON = [] # pts에 저장된 포인트를 json형태로 저장
file_path = 'C:/Users/USER/Desktop/get_coordinates_with_mouse_using_opencv/1124/vidoes_frame_4_sample2_7.json' #json으로 저장하기 위한 파일경로
base_dir = 'C:/Users/user/Desktop/get_coordinates_with_mouse_using_opencv/1124/2/7/' # 이미지 읽어오는 경로 0~7, 1~7 다 넘김
...
def draw_roi(event, x, y, flags, param):  # roi검출을 위한 함수 정의
    ...
    if event == cv.EVENT_LBUTTONDOWN:  # 마우스 왼쪽버튼을 클릭하면
        ...
        # print(img.shape)
        # print(width,height)
        # print(int(y*(img.shape[0]/height)))
        print('이미지:', img_file)
