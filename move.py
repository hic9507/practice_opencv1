...
def point_gen(M, N, space, w_num_of_pixel,  h_num_of_pixel, shape):
    x1, y1 = M[0][0], M[0][1]
    x2, y2 = N[0][0], N[0][1]

    if ((x1 != x2) and (y1 == y2)):
    # for i in range(1, int(num_of_pixel) + 1):
        for i in range(1, int(w_num_of_pixel) + 1):
            # for j in range(1, int(h_num_of_pixel) + 1):
            if ((x1 != x2) and (y1 == y2)): #(A,B) or (C,D)
                M.append([x1, y1 + space * i]) # space > 3/5
            ...
            if M[int(len(M) - 1)][0] >= 3/5*shape[1]:  ####오른쪽 튀어나감
                del M[int(len(M) - 1)]

            if M[0][0] < 0:  ####위쪽 튀어나감
                del M[0]

            if M[int(len(M) - 1)][1] >= 3/5*shape[0]:  ####아랫쪽 튀어나감
                del M[int(len(M) - 1)]
            ...
            if N[0][0] < shape[1]-shape[1]*3/5:  ####어ㅣㄴ쪽 튀어나감
                del N[0]

            if N[int(len(N) - 1)][1] >= 3/5*shape[0]:  ####아랫쪽 튀어나감
                del N[int(len(N) - 1)]
    if ((x1 == x2) and (y1 != y2)):
        for j in range(1, int(h_num_of_pixel) + 1):
            if ((x1 == x2) and (y1 != y2)):
                M.append([x1 + space * j, y1])
                N.append([x2 + space * j, y2])
                M.insert(0, [x1 - space * j, y1]) # 이게 문제인 듯
                N.insert(0, [x2 - space * j, y2])
            # print('CIVA')
            if M[0][1] < 0:  ####왼쪽 튀어나감
                del M[0]
            ...
            if N[0][1] < 0:  ####왼쪽 튀어나감
                del N[0]
            if N[int(len(N) - 1)][0] >= 3/5*shape[1]:  ####오른쪽 튀어나감
                del N[int(len(N) - 1)]
...
image = cv2.imread('false_cc10_jpg.rf.dad38dec4b48e5f0b66b10cb9d329671.jpg')
image_gray = cv2.imread('false_cc10_jpg.rf.dad38dec4b48e5f0b66b10cb9d329671.jpg', cv2.IMREAD_GRAYSCALE)


# image = cv2.imread('test5-103.jpg_black.jpg')
# image_gray = cv2.imread('test5-103.jpg_black.jpg', cv2.IMREAD_GRAYSCALE)
...
w_num_of_pixel = int(3/5 * (image.shape[1]))
h_num_of_pixel = int(3/5 * (image.shape[0]))
...
Z, X = point_gen(copy.deepcopy(A), copy.deepcopy(B), space, w_num_of_pixel, h_num_of_pixel, image.shape)
print(detector(Z, X, image, image_gray))

Z, X = point_gen(copy.deepcopy(A), copy.deepcopy(C), space, w_num_of_pixel, h_num_of_pixel,image.shape)
print(detector(Z, X, image, image_gray))

Z, X = point_gen(copy.deepcopy(B), copy.deepcopy(D), space, w_num_of_pixel, h_num_of_pixel, image.shape)
print(detector(Z, X, image, image_gray))

Z, X = point_gen(copy.deepcopy(C), copy.deepcopy(D), space, w_num_of_pixel, h_num_of_pixel, image.shape)
print(detector(Z, X, image, image_gray))
