...
def draw_roi(event, x, y, flags, param):  # roi검출을 위한 함수 정의
    ...
    if event == cv.EVENT_LBUTTONDOWN:  # 마우스 왼쪽버튼을 클릭하면
        pts.append((int(x * img.shape[1]/1080), int(y * img.shape[0]/1080)))  # pts에 (x,y)좌표를 추가한다
        print(width,height)
        print('이미지:', img_file)
        print('포인트 #%d 좌표값(%d,%d)' % (len(pts), int(x), int(y)))  # 정상적으로 추가되는지 출력으로 확인
        resultforJSON.append({'image': img_file,
                            'point': [len(pts)],
                            'coordinate': [[int(x*(img2.shape[1]/1080)), int(y*(img2.shape[0]/1080))]]})
        ...
        if len(pts) == 4:
            with open(file_path, 'w') as outfile:  # resultforJSON에 저장된 내용을 json파일로 추출
                ...
                cv.imwrite('C:/Users/user/Desktop/get_coordinates_with_mouse_using_opencv/1124/mask/{}'.format(img_file), mask_black)
        # cv.imshow('ROI', ROI)
        # cv.imshow('black', mask_black)
        # cv.waitKey(0)
    if len(pts) > 0:  # 포인트를 '원'으로 표시
        cv.circle(img2, (int(pts[-1][0]/(img.shape[1]/height)) , int(pts[-1][1]/(img.shape[0]/width))), 3, (0, 0, 255), -1)
        # cv.circle(img2, (x,y), 3, (0, 0, 255), -1)

    if len(pts) > 1:
        for i in range(len(pts) - 1):
            cv.circle(img2, (int(pts[i][0]/(img.shape[1]/height)), int(pts[i][1]/(img.shape[0]/width))), 5, (0, 0, 255), -1)
        cv.line(img=img2, pt1=(int(pts[i][0]/(img.shape[1]/height)), int(pts[i][1]/(img.shape[0]/width))), pt2=(int(pts[i+1][0]/(img.shape[1]/height))  , int(pts[i+1][1]/(img.shape[0]/width))), color=(255, 255, 255), thickness=2)
